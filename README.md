...
[![Version](https://img.shields.io/badge/version-7.2.31-6E66ED)](https://github.com/cosmotown/emby-vision-hub/releases)
    ...
    services:
      ...
      emby-vision-hub:
        ...
        environment:
          ...
          - CONTAINER_NAME=emby-vision-hub            # 标识当前 EVH 服务；内建更新仍会严格验证部署身份
...
- 内建更新器支持已验证为当前 EVH 自身的独立 Docker 容器，以及身份完整、单实例、声明官方 `tzyzero186/emby-vision-hub:latest` 的 Portainer Compose Stack。两者都要求持久可写 `/config`、可写 Docker socket 和有效 Docker Healthcheck；更新器固定正式 Release 对应的 image ID 后事务性替换当前容器。
- 普通 Docker Compose、固定版本 tag 的 Portainer Stack、Portainer 非 Compose 部署、1Panel、Swarm、Kubernetes、Nomad 均须通过原部署管理器升级。多实例、Compose 身份不完整、NFS/SMB/未知锁文件系统、tmpfs 挂载及特殊 volume options/static network 继续 fail closed。
- `AMBIGUOUS` 表示对象归属或 Docker 操作结果无法证明：停止自动操作并保留现场，不删除冲突对象，也不会把拉取成功当升级成功。首次安装本版必须通过外部管理器完成；新更新器从下一次升级开始生效。
- 使用内建更新必须持久化可写 `/config`，并以 bind mount 提供可写 `/var/run/docker.sock`（该接口具有宿主机管理权限；不使用内建更新时无需挂载）。Portainer Stack 更新会保留并复核 Compose 身份与配置 hash，同时更新 Compose image identity，确保后续 Stack reconcile 不回滚成功结果。更新器会校验新容器的 image ID、`APP_VERSION`、health 和关键运行配置；条件失败时尝试恢复旧容器，无法证明归属时停止自动操作。
